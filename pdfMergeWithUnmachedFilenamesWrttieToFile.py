...
# Driver code
	...
	# File with matched PDF and merged, Change output.pdf to whatever name you wanr	
    with open("output.pdf", 'wb') as pdfWFh:  
       pdfWriter.write(pdfWFh)

    # PDF  File unmatched files names
    unmatchedFiles = open("unmatchedFileNames.txt", "w")
    for fileName in unmatchedItems:
       unmatchedFiles.write(fileName+'\n')
